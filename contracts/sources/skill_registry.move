...
/// Structural key used for pinning skills to the ANIMA object dynamic field layout
...
public fun authorize_skill(
    ...
    let key = SkillKey { name: skill_name };
...
public fun revoke_skill(agent: &mut ANIMA, skill_name: String, cap: &OwnerCap) {
    protocol::assert_not_paused(agent);
    protocol::check_owner_cap(agent, cap);
