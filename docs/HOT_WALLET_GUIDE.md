...
## Step 1: Spin Up the Hot Wallet Address
...
export async function autoFundOperator(
  ...
  try {
    ...
    if (totalBalance < minBalance) {
      console.log(
        `⚠️  Operator ${operatorAddress} has low SUI balance (${Number(totalBalance) / 1e9} SUI). Seeding 0.05 SUI...`,
      );
      ...
      console.log(
        `✓ Successfully funded operator address with 0.05 SUI for gas! Digest: ${response.digest}`,
      );
    } else {
      console.log(
        `✓ Operator address has sufficient gas: ${Number(totalBalance) / 1e9} SUI`,
      );
    }
  } catch (error) {
...
- **Why it happens:** The sender (`ctx.sender()`) of the transaction block did not match the registered `operator_address` on the agent object. Even if the key is in your keystore, the CLI will default to your _active_ address as the sender if not specified.
